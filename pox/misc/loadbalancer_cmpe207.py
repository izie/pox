...
class iplb (object):
  ...
  def _pick_server (self, key, inport):
    ...
    while self.is_not_found:
      next_svr = self.get_next_server()
      for key in self.live_servers.iteritems():
        if(key[1][1] == (next_svr+1)):
          self.is_not_found = False
          self.selected_server = key[0]
          break
      if(not self.is_not_found):
        break

    log.info("RR selected server : "+str(self.selected_server))
